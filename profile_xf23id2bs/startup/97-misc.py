...
#        df.to_csv('~/User_Data/Hunt/Prop_302802/Scan_%d.csv' % scanid, columns=['pgm_energy_readback', 'time', 'sclr_ch2', 'sclr_ch3', 'sclr_ch4', 'norm_ch4', 'vortex_mca_rois_roi3_count', 'vortex_mca_rois_roi4_count' ], index=False)
        df.to_csv('~/Documents/May/Scan_%d.csv' % scanid, columns=['pgm_energy_readback', 'time', 'sclr_ch3', 'sclr_ch4', 'norm_ch4', 'vortex_mca_rois_roi3_count', 'vortex_mca_rois_roi4_count' ], index=False)
...
def save_xas_csv_short(first_id, last_id):
        for scanid in range(first_id,last_id+1,1):
                ...
                #fn = 'csv_data/Scan_{scan_id}.csv'.format(db[scanid].start)
                df.to_csv('~/Documents/Yildiz/Scan_%d.csv' % scanid, columns=['pgm_energy_readback', 'sclr_ch3', 'sclr_ch4', 'norm_ch4', 'vortex_mca_rois_roi4_count', 'vortex_mca_rois_roi3_count', 'ring_curr'])

def save_nicks_csv(first_id, last_id):
        for scanid in range(first_id,last_id+1,1):
                df = db.get_table(db[scanid])
                #fn = 'csv_data/Scan_{scan_id}.csv'.format(db[scanid].start)
                df.to_csv('~/User_Data/Quackenbush_XAS/July2018/Scan_%d.csv' % scanid, columns=['time','appes_x','appes_y', 'sclr_ch2', 'sclr_ch3', 'sclr_ch4', 'vortex_mca_rois_roi2_count', 'vortex_mca_rois_roi3_count', 'vortex_mca_rois_roi4_count'])
...
def plot_gapscans(scanid1,scanid2,label):
        plt.figure(label)
        label = plt.gca()
        for i in range (scanid1, scanid2+1):
                df1 = db.get_table(db[i])
                df1.plot(x = 'epu1_gap_readback', y = 'sclr_ch2', label = str(i), ax=label)
...
def PD_count():
    ...
    yield from bp.count(dets, num=None, delay=60)
...
REF_EDGES = {'Al' : {'energy': 1560 , 'epu_table': 3 , 'vortex_low': 3200 , 'vortex_high': 4000},
             'Cu_pileup' : {'energy': 932  , 'epu_table': 2 , 'vortex_low': 1500  , 'vortex_high': 2200},
             'Cu' : {'energy': 932  , 'epu_table': 2 , 'vortex_low': 900  , 'vortex_high': 1200},
             'O'  : {'energy': 540  , 'epu_table': 2 , 'vortex_low': 450  , 'vortex_high': 650},
             'Ni' : {'energy': 851  , 'epu_table': 2 , 'vortex_low': 1000 , 'vortex_high': 1200},
             ...
             'Co' : {'energy': 780  , 'epu_table': 2 , 'vortex_low': 700  , 'vortex_high': 950},
             'Mn' : {'energy': 640  , 'epu_table': 2 , 'vortex_low': 800  , 'vortex_high': 900},
